...
do
    ...
    local function createRadar()
        ...
        for _, dist in ipairs({150, 300}) do
    ...
    local function updateRadar()
        if not FuncState.RadarEnabled then
            ...
            for _, point in pairs(radarPoints) do point:Destroy() end
            radarPoints = {}
            return
        end
        ...
        if not myRoot then radarFrame.Visible = false return end
        radarFrame.Visible = true
        local myPos = myRoot.Position
        local radarSize = radarFrame.AbsoluteSize
        if ((radarSize.X == 0) or (radarSize.Y == 0)) then return end
        local maxDist = radarWorldRange
        for _, point in pairs(radarPoints) do point:Destroy() end
        radarPoints = {}
                        ...
                        if hb then
                            ...
                            if FuncState.HealthBarEnabled then
                                ...
                                if (myRoot and head) then
                                    ...
                                    hb.Size = UDim2.new(0, math.floor(5 * scale), 0, math.floor(40 * scale))
                                end
                            end
                        end
                        local hrp = char:FindFirstChild("HumanoidRootPart")
...
do
    ...
    local function cleanupFly()
        ...
        flyConnections = {}
        for _, inst in ipairs(flyInstances) do
        ...
        if char then
            ...
            if torso then
                local bg = torso:FindFirstChild("FlyBodyGyro")
                if bg then bg:Destroy() end
                local bv = torso:FindFirstChild("FlyBodyVelocity")
    ...
    execBtn.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            local ok, err = pcall(function()
                ...
                flyGui.Name = "shible_Fly"
                flyGui.ResetOnSpawn = false
                flyGui.Parent = LocalPlayer:WaitForChild("PlayerGui")
                table.insert(flyInstances, flyGui)

                local f = Instance.new("Frame", flyGui)
                ...
                table.insert(flyInstances, down)
                ...
                onof.Selectable = false
                table.insert(flyInstances, onof)
                ...
                table.insert(flyInstances, plus)
                ...
                mine.Selectable = false
                table.insert(flyInstances, mine)
                ...
                xbtn.Selectable = false
                table.insert(flyInstances, xbtn)
                ...
                table.insert(flyInstances, mini)
                ...
                mini2.Selectable = false
                table.insert(flyInstances, mini2)
                ...
                local function stopFly()
                    ...
                    if bgObj then
                        pcall(function() bgObj:Destroy() end)
                        bgObj = nil
                    end
                    if bvObj then
                        pcall(function() bvObj:Destroy() end)
                        bvObj = nil
                    end
                    resetHum()
                end
                ...
                local function startFly()
                    ...
                    if torso then
                        ...
                        table.insert(flyConnections, renderConn)
                    end
                end

                local charConn = LocalPlayer.CharacterAdded:Connect(function(c)
                    nowe = false
                    onof.Text = "飞"
                    chr = c
                ...
                table.insert(flyConnections, charConn)
...
do
    ...
    hdr.TextXAlignment = Enum.TextXAlignment.Left
    y = y + 36
    createToggle(p, y, "旋转", function() return FuncState.SpinEnabled end, function(v) FuncState.SpinEnabled = v end)
    y = y + 50
    createSlider(p, y, "旋转倍数 (10-999)", 10, 999, 50, function(v) FuncState.SpinSpeed = v end)
    y = y + 50
    createToggle(p, y, "甩飞所有", function() return FuncState.FlingLoaded end, function(v)
        if v then
            ...
            if not FuncState.FlingLoaded then toggleSetters["甩飞所有"](false) end
        else
...
do
    ...
    closeBtnAction.MouseButton1Click:Connect(function()
        ...
        if char and char:FindFirstChildOfClass("Humanoid") then
            local hum = char:FindFirstChildOfClass("Humanoid")
            pcall(function() hum:LoadAnimation(Instance.new("Animation")):Stop() end)
        end
...
do
    ...
    hdr.TextXAlignment = Enum.TextXAlignment.Left
    y = y + 36
    createToggle(p, y, "开启预防检测", function() return FuncState.AntiDetect end, function(v)
    ...
    y = y + 46
    createToggle(p, y, "隐藏执行痕迹", function() return FuncState.HideTraces end, function(v)
        FuncState.HideTraces = v
        if v then hideExecutionTraces() end
    end)
    y = y + 46
    createSlider(p, y, "防御等级 (1-3)", 1, 3, 2, function(v)
        FuncState.DefenseLevel = v
    end)
    local info = Instance.new("TextLabel", p)
    info.Text = "等级1:基础防护 | 等级2:中级加强 | 等级3:激进清理(可能影响性能)"
    info.Font = Enum.Font.Gotham
    info.TextSize = 12
    info.TextColor3 = Theme.TextSecondary
    info.BackgroundTransparency = 1
    info.Position = UDim2.new(0, 16, 0, y + 36)
    ...
    shutdownBtn.MouseButton1Click:Connect(function() game:Shutdown() end)
    y = y + 42
...
local function deepClearTable(t)
    if type(t) ~= "table" then return end
    local keys = {"AC_Enabled", "BanDetect", "CheatDetect", "ExploitDetect", "InjectDetect", "AntiCheat", "AC", "CheckAC", "BanCheck", "ExploitCheck"}
    for _, key in ipairs(keys) do
        t[key] = nil
    end
    for k, v in pairs(t) do
        if type(v) == "table" and k ~= "_shible_Fake" then
            deepClearTable(v)
        end
    end
end
...
local function cleanAntiCheatMemory()
    ...
    pcall(function()
        if _ENV then deepClearTable(_ENV) end
        if getfenv and getfenv(0) then deepClearTable(getfenv(0)) end
    end)
    pcall(function()
        if getgenv then
...
local function hideExecutionTraces()
    pcall(function()
        if getgc then
            local gc = getgc(true)
            for i, v in ipairs(gc) do
                if type(v) == "string" then
                    ...
                    if lv:find("shible") or lv:find("exploit") or lv:find("inject") or lv:find("cheat") then
                        if lv:len() < 100 then
                            v = ""
                        end
                    end
...
local function hookDetectionFunctions()
    pcall(function()
        local hooks = {"check", "ban", "scan", "detect", "anti", "report"}
        for _, name in ipairs(hooks) do
            if _G[name] and type(_G[name]) == "function" then
                local old = _G[name]
                _G[name] = function(...) return nil end
            end
            if shared and shared[name] and type(shared[name]) == "function" then
                shared[name] = function(...) return nil end
            end
        end
    end)
end

local function blockDetectionRemotes()
    pcall(function()
        for _, item in ipairs(game:GetDescendants()) do
            if item:IsA("RemoteEvent") or item:IsA("RemoteFunction") then
                local name = item.Name:lower()
                if name:find("check") or name:find("detect") or name:find("anti") or name:find("ban") or name:find("scan") then
                    pcall(function()
                        if item:IsA("RemoteEvent") then
                            item.OnServerEvent = function() end
                            item.OnClientEvent = function() end
                        elseif item:IsA("RemoteFunction") then
                            item.OnServerInvoke = function() return nil end
    ...
    pcall(function()
        for _, item in ipairs(game:GetDescendants()) do
            if item:IsA("Script") or item:IsA("LocalScript") then
                local name = item.Name:lower()
                if name:find("anti") or name:find("detect") or name:find("check") or name:find("ban") or name:find("scan") then
...
local function fakeACEnvironment()
    pcall(function()
        ...
        local fakeFuncs = {
            CheckAC = false,
            BanCheck = false,
            AntiCheatCheck = false,
            ExploitCheck = false
        }
        for name, val in pairs(fakeFuncs) do
            _G[name] = function() return val end
            if shared then shared[name] = function() return val end end
        end
        _G.AC_Enabled = false
        _G.AntiCheat = nil
        _G.AC = nil
        if shared then
            shared.AC_Enabled = false
            ...
            if child:IsA("RemoteEvent") or child:IsA("RemoteFunction") then
                ...
                if name:find("check") or name:find("detect") or name:find("anti") or name:find("ban") or name:find("scan") then
                    if child:IsA("RemoteEvent") then
                        ...
                        child.OnClientEvent = function() end
                    elseif child:IsA("RemoteFunction") then
                        child.OnServerInvoke = function() return nil end
...
local function startAntiDetect()
    ...
    pcall(function()
        if FuncState.DefenseLevel >= 2 then
            cleanAntiCheatMemory()
        end
        blockDetectionRemotes()
    ...
    while antiDetectRunning do
        ...
        if FuncState.AntiDetect then
            pcall(function()
                ...
                if FuncState.DefenseLevel >= 3 then
                    for _, item in ipairs(workspace:GetDescendants()) do
                        if item:IsA("Script") and item.Name:lower():find("anti") then
                            item.Disabled = true
                        end
                    end
                end
...
local function stopAntiDetect()
    ...
    Notify("shible", "防检测已停止", 2)
end

local function enhancedAdminDetect()
    pcall(function()
        local admins = {}
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr == LocalPlayer then continue end
            local name = plr.Name:lower()
            local adminKeywords = {"admin", "owner", "creator", "dev", "moderator", "staff", "gm", "helper"}
        ...
        if #admins > 0 then
            Notify("shible", "⚠️ 检测到管理员: " .. table.concat(admins, ", "), 5)
        end
    end)
end

local function enhancedACBypass()
    pcall(function()
...
local function enhancedGroupBypass()
    pcall(function()
        _G.GroupVerified = true
        _G.GroupCheck = function() return true end
        _G.RequireGroup = function() return true end
        if shared then
            shared.GroupVerified = true
            shared.GroupCheck = function() return true end
        end
    end)
end
...
task.spawn(function()
    while antiDetectRunning do
        ...
        pcall(function()
            ...
            if FuncState.BypassAC then enhancedACBypass() end
        end)
    end
end)
...
Players.PlayerAdded:Connect(function(plr)
    task.wait(3)
    if FuncState.AdminDetect then
        pcall(enhancedAdminDetect)
    end
end)

_G._shible_AntiDetect = {
    start = startAntiDetect,
    stop = stopAntiDetect,
    isRunning = function() return antiDetectRunning end
}
