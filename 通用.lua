...
do
    ...
    y = y + 46
    createToggle(p, y, "隐藏执行痕迹", function()
        return FuncState.HideTraces or false
    end, function(v)
        ...
        for i, v in ipairs(gc) do
            if type(v) == "table" then
                local suspiciousKeys = {
                    "_G", "AntiCheat", "antiCheat", "AC_Enabled", "BanDetect",
                    "CheatDetect", "ExploitDetect", "ScriptCheck", "InjectDetect",
                    "AC", "antiban", "AntiBan", "Detect", "detect",
                    "Log", "log", "Report", "report", "Webhook", "webhook",
                    "Telemetry", "Analytics", "Monitor", "monitor",
                    "Track", "track", "Flag", "flag", "Alert", "alert"
                }
                
                for _, key in ipairs(suspiciousKeys) do
                    if v[key] ~= nil then
                        if type(v[key]) == "function" or type(v[key]) == "table" or type(v[key]) == "userdata" then
                            v[key] = nil
                        end
                    end
                end
                ...
                if mt then
                    local dangerousMetas = {
                        "__index", "__newindex", "__call", "__metatable",
                        "__mode", "__name", "__tostring"
                    }
                    local safe = true
                    for _, metaKey in ipairs(dangerousMetas) do
                        if mt[metaKey] then
                            safe = false
                            break
                        end
            ...
            elseif type(v) == "function" then
                local funcStr = tostring(v)
                local dangerousPatterns = {
                    "AntiCheat", "detect", "ban", "cheat", "exploit",
                    "inject", "script", "check", "verify", "validate",
                    "report", "log", "webhook", "telemetry", "analytics"
                }
                for _, pattern in ipairs(dangerousPatterns) do
                    if funcStr:find(pattern, 1, true) then
                        pcall(function()
                            local env = getfenv(v)
                            if env then
                                for k, _ in pairs(env) do
                                    local lowerK = string.lower(k)
                                    if lowerK:find("anti") or lowerK:find("detect") or 
                                       lowerK:find("ban") or lowerK:find("cheat") or
                                       lowerK:find("exploit") or lowerK:find("script") then
                                        env[k] = nil
                                    end
                                end
                            end
...
local function hideExecutionTraces()
    pcall(function()
        ...
        if rawget(_G, "_G") then
            rawset(_G, "_G", nil)
        end
        
        local env = getfenv(0)
        local dangerous = {"script", "loadstring", "getfenv", "setfenv", "debug", "pcall"}
...
local function hookAllRemotes()
    pcall(function()
        local remotePatterns = {
            "check", "detect", "anti", "ban", "report", "log",
            "webhook", "telemetry", "analytics", "monitor", "track",
            "flag", "alert", "verify", "validate", "auth", "security"
        }
            ...
            if item:IsA("RemoteEvent") or item:IsA("RemoteFunction") then
                ...
                if shouldHook then
                    if item:IsA("RemoteEvent") then
                        local oldEvent = item.OnServerEvent
                        if oldEvent then
                    ...
                    elseif item:IsA("RemoteFunction") then
                        ...
                        if oldInvoke then
                            item.InvokeServer = function() return nil end
                        end
                    end
                end
            end
...
local function blockScripts()
    pcall(function()
        local blockPatterns = {
            "anti", "detect", "check", "ban", "report", "log",
            "webhook", "telemetry", "analytics", "monitor", "track",
            "flag", "alert", "verify", "validate", "auth", "security",
            "ac", "cheat", "exploit", "inject", "script"
        }
    ...
    pcall(function()
        if not _G._shible_FakeAC then
            _G._shible_FakeAC = {
                ...
                Security = { Enabled = false, Level = 0 }
            }
        end
        ...
        local fakeFunctions = {
            ...
            ExploitCheck = function() return false end,
            InjectCheck = function() return false end,
            ScriptCheck = function() return false end,
            VerifyUser = function() return true end,
            ValidateClient = function() return true end,
            AuthCheck = function() return true end,
            SecurityCheck = function() return true end
        }
        
        for name, func in pairs(fakeFunctions) do
            if not _G[name] then
                _G[name] = func
            end
        end
        ...
        local cleanVars = {
            "AC_Enabled", "AntiCheatEnabled", "BanDetect", 
            "AntiCheat", "AC", "CheatDetect", "ExploitDetect",
            "InjectDetect", "ScriptDetect", "CheckEnabled",
            "SecurityEnabled", "MonitorEnabled", "TrackEnabled",
            "DetectionEnabled", "ScanEnabled", "VerifyEnabled"
        }
        
        for _, var in ipairs(cleanVars) do
            if rawget(_G, var) then
                if type(rawget(_G, var)) == "boolean" then
                    rawset(_G, var, false)
                else
                    rawset(_G, var, nil)
                end
...
local function blockNetworkRequests()
    pcall(function()
        local httpService = game:GetService("HttpService")
        if httpService then
            ...
            if oldGet then
                httpService.GetAsync = function(self, url, ...)
                    local blockedUrls = {
                        "discord.com/api/webhooks", "webhook", "webhook.site",
                        "discordapp.com/api/webhooks", "ptb.discord.com/api/webhooks",
                        "canary.discord.com/api/webhooks", "api.telegram.org",
                        "hooks.slack.com", "log", "report", "analytics"
                    }
                    for _, blocked in ipairs(blockedUrls) do
                        if url and url:find(blocked, 1, true) then
                            return ""
                        end
                    end
...
local function clearInjectionPoints()
    pcall(function()
        local dangerousGlobals = {
            "getgenv", "setgenv", "getfenv", "setfenv", "getrenv",
            "getreg", "getgc", "getrawmetatable", "setrawmetatable",
            "syn", "synapse", "krnl", "sentinel", "scriptware",
            "iota", "xeno", "trigon", "lunar", "vape", "cheat"
        }
        ...
        for _, name in ipairs(dangerousGlobals) do
            ...
            if rawget(getfenv(0), name) then
                rawset(getfenv(0), name, nil)
            end
        end
...
local function enhancedAntiDetect()
    pcall(function()
        deepGCClean()
        hookAllRemotes()
        blockScripts()
        ...
        local cleanConn = RunService.Heartbeat:Connect(function()
            ...
            if tick() % 5 < 0.1 then
                ...
                pcall(blockNetworkRequests)
                pcall(clearInjectionPoints)
                if FuncState.HideTraces then
        ...
        table.insert(antiDetectConnections, cleanConn)
        
        Notify("shible", "增强防检测已启动 (8层防护)", 2)
    end)
end
...
local function enhancedAdminDetect()
    pcall(function()
        ...
        for _, plr in ipairs(allPlayers) do
            ...
            local adminKeywords = {
                "admin", "owner", "creator", "dev", "moderator", 
                "staff", "gm", "helper", "support", "manager",
                "super", "head", "lead", "senior", "vip", "premium"
            }
            ...
            if not isAdmin and plr.Team then
                ...
                for _, keyword in ipairs(adminKeywords) do
                    if teamName:find(keyword, 1, true) then
                        isAdmin = true
                        break
                    end
                end
            ...
            if not isAdmin and plr.MembershipType then
                if plr.MembershipType == Enum.MembershipType.Admin then
                    isAdmin = true
                end
            end
...
local function enhancedACBypass()
    pcall(function()
        ...
        for _, var in ipairs(acVars) do
            if rawget(_G, var) then
                rawset(_G, var, false)
            end
            if rawget(getfenv(0), var) then
                rawset(getfenv(0), var, false)
            end
        end
        
        local acFunctions = {
            "CheckAC", "AntiCheatCheck", "BanCheck", "ExploitCheck",
            "InjectCheck", "ScriptCheck", "VerifyUser", "ValidateClient",
            "AuthCheck", "SecurityCheck", "DetectCheat", "ScanExploit"
        }
        
        for _, func in ipairs(acFunctions) do
        ...
        for _, v in pairs(getgc(true)) do
            if type(v) == "table" then
                local mt = getmetatable(v)
                if mt then
                    local acKeys = {"AC", "AntiCheat", "BanDetect", "CheatDetect"}
...
local function enhancedGroupBypass()
    pcall(function()
        local groupKeys = {
            "GroupCheck", "RequireGroup", "GroupVerify", 
            "GroupVerified", "CheckGroup", "VerifyGroup",
            "ValidateGroup", "AuthGroup", "GroupAuth"
        }
        ...
        for _, key in ipairs(groupKeys) do
            ...
            if rawget(getfenv(0), key) then
                rawset(getfenv(0), key, nil)
            end
        end
        ...
        _G.ValidateGroup = function() return true end
