...
do
    ...
    -- 修改：血量条显示在角色左侧（StudsOffset 改为负X）
    local function createHealthBar(head)
        local bb = Instance.new("BillboardGui")
        bb.Name = "ESP_HB"
        bb.Size = UDim2.new(0, 6, 0, 60)          -- 竖向条形
        bb.StudsOffset = Vector3.new(-2, 0.5, 0)   -- 左侧偏移（X负值），略上浮
        bb.Adornee = head
        bb.AlwaysOnTop = true
        bb.MaxDistance = 500
        bb.Enabled = false
                        ...
                        if hb then
                            ...
                            if FuncState.HealthBarEnabled then
                                ...
                                if (myRoot and head) then
                                    ...
                                    hb.Size = UDim2.new(0, math.floor(5 * scale), 0, math.floor(55 * scale))
                                end
                            end
                        end
                        local hrp = char:FindFirstChild("HumanoidRootPart")
...
do
    ...
    y = y + 46
    createToggle(p, y, "隐藏执行痕迹", function() return FuncState.HideTraces or false end, function(v)
