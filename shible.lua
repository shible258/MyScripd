...
local function SafeLoad(url, name)
    ...
    for attempt = 1, 4 do
        ...
        if body and #body > 10 then break end
        pcall(function()
    ...
    if (not body or (#body < 10)) then
        warn("[SafeLoad] " .. name .. " 下载失败，body为空或过短")
        Notify("shible", name .. " 加载失败，请检查网络或URL", 3)
        return false
    end
...
do
    ...
    createToggle(p, y, "启用受击范围", function()
        ...
        if v then
            if not hitboxUpdateConn then
                hitboxUpdateConn = RunService.Heartbeat:Connect(function()
                    if not FuncState.HitboxEnabled then
                        return
                    end
                    ...
                    for _, plr in ipairs(Players:GetPlayers()) do
                        ...
                        if not hl then
                            ...
                            hl.Enabled = true
                            hl.Parent = char
                            hitboxHighlights[plr] = hl
                        end
                        for _, part in ipairs(char:GetDescendants()) do
                            if part:IsA("BasePart") then
                                local orig = hitboxSizes[part]
                                if not orig then
                                    orig = part.Size
                                    hitboxSizes[part] = orig
                                end
                                part.Size = orig * scale
                            end
                        end
                    end
                    for plr, hl in pairs(hitboxHighlights) do
                        if not plr.Character or not plr.Character.Parent then
                            hl:Destroy()
                            hitboxHighlights[plr] = nil
                        end
                    end
                    for part, orig in pairs(hitboxSizes) do
                        if not part.Parent or not part.Parent.Parent then
                            hitboxSizes[part] = nil
                        end
                    end
                end)
            end
            ...
            for _, hl in pairs(hitboxHighlights) do
