...
local function SafeLoad(url, name, fallbackUrls)
    ...
    for attempt, currentUrl in ipairs(urls) do
        ...
        -- Delta 专用: 优先使用 game:HttpGet
        pcall(function()
            body = game:HttpGet(currentUrl)
        end)

        -- 备选: 如果 game:HttpGet 失败，尝试其他方法
...
local function createSubZhuiDrawings()
    ...
    aimLine.Thickness = 2
    aimLine.Color = Color3.new(1,1,1)
    ...
    circle.Visible = true
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            local dirToHead = (headPos - cameraPos).Unit
            local angle = math.deg(math.acos(cameraLook:Dot(dirToHead)))
            if angle <= fovSize/2 then
                if wallCheckEnabled then
                    local params = RaycastParams.new()
                    params.FilterType = Enum.RaycastFilterType.Blacklist
                    params.FilterDescendantsInstances = {LocalPlayer.Character, char}
                    local result = workspace:Raycast(cameraPos, dirToHead * (headPos - cameraPos).Magnitude, params)
                    if result then continue end
                end
                local dist = (headPos - cameraPos).Magnitude
                table.insert(enemies, {plr = plr, head = head, distance = dist, pos = headPos})
            end
        end
        ...
        if aimLine then
            if target then
                local screenPos, onScreen = camera:WorldToScreenPoint(target.pos)
                if onScreen then
                    aimLine.From = center
        ...
        if target then
            local aimPos = target.pos
            if math.random(1,100) > headshotChance then
                local rootPart = target.plr.Character:FindFirstChild("HumanoidRootPart")
                if rootPart then
                    aimPos = rootPart.Position
                end
            end
            local newCF = CFrame.new(cameraPos, aimPos)
            camera.CFrame = newCF
        end
    end
    ...
    y = y + 56
    createSlider(p, y, "爆头概率", 0, 100, 100, function(v)
        headshotChance = v
    end)
end
...
do
    ...
    local function createVisual(char, size)
        local hrp = char:FindFirstChild("HumanoidRootPart")
        ...
        frame.BorderTransparency = 0.1
        frame.ZIndex = 10
        corner(frame, 4)
        return bgui
    end

    local function applyToCharacter(char, size)
        ...
        for _, part in ipairs(char:GetDescendants()) do
            if part:IsA("BasePart") then
                ...
                pcall(function()
                    part.Size = orig * scaleFactor
                end)
            end
        end
        local vis = createVisual(char, size)
        if vis then
            visuals[char] = vis
        end
    end
    ...
    local function applyToAll()
        for _, plr in ipairs(Players:GetPlayers()) do
            ...
            local char = plr.Character
            if char then
                applyToCharacter(char, hitboxSize)
            end
        end
    end
    ...
    local function restoreAll()
        local chars = {}
        for char, _ in pairs(visuals) do table.insert(chars, char) end
    ...
    createToggle(p, y, "启用受击范围", function() return hitboxEnabled end, function(v)
        hitboxEnabled = v
        if v then
            applyToAll()
        else
    ...
    local info = Instance.new("TextLabel", p)
    info.Text = "透明白色框 = 受击范围，攻击框内即可命中本体"
    info.Font = Enum.Font.Gotham
    info.TextSize = 12
    info.TextColor3 = Theme.TextSecondary
    info.BackgroundTransparency = 1
...
local function cleanupAll()
    if _G._cleanupHitbox then
        pcall(_G._cleanupHitbox)
    end
