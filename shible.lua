...
do
    ...
    y = y + 46
    createToggle(p, y, "平滑自瞄", function() return FuncState.SmoothAim end, function(v)
        FuncState.SmoothAim = v
    end)

    y = y + 46
    createSlider(p, y, "平滑强度 (1-10)", 1, 10, 5, function(v)
        FuncState.SmoothAimStrength = v
    end)

    y = y + 46
    local shibleAimLoaded = false
    local shibleAimURL = "https://raw.githubusercontent.com/ylt410/roblox-Script/refs/heads/main/ye%20aimbot"
    createToggle(p, y, "shible自瞄", function() return FuncState.ShibleAimLoaded end, function(v)
        if v then
            FuncState.ShibleAimLoaded = SafeLoad(shibleAimURL, "shible自瞄")
            if not FuncState.ShibleAimLoaded then warn("[shible自瞄] 加载失败") end
        else
            pcall(function() getgenv().AimbotEnabled = false end)
            FuncState.ShibleAimLoaded = false
        end
    end)
end
...
do
    ...
    y = y + 46
    createToggle(p, y, "队伍检测", function() return FuncState.TeamCheck end, function(v)
        FuncState.TeamCheck = v
    end)
            ...
            if ((plr ~= LocalPlayer) and plr.Character) then
                ...
                if hrp then
                    ...
                    if (horizontalDist <= maxDist) then
                        ...
                        local pixelOffsetX = normalizedDist * radarRadiusPixels * math.cos(angle)
                        local pixelOffsetY = normalizedDist * radarRadiusPixels * math.sin(angle)
...
do
    ...
    y = y + 50
    createToggle(p, y, "飞车", function()
        return FuncState.FlyCarEnabled
    end, function(v)
        FuncState.FlyCarEnabled = v
    end)
    ...
    createToggle(p, y, "闪电尖兵大招", function() return FuncState.TPEnabled end, function(v)
        FuncState.TPEnabled = v
        if v then
            Notify("瞬移功能", "已开启", 2)
        else
            Notify("瞬移功能", "已关闭", 2)
        ...
        if v then
            ...
            if char then
                ...
                conn = char.DescendantAdded:Connect(function(part)
                    if part:IsA("BasePart") and FuncState.Noclip then
                        part.CanCollide = false
                    end
                end)
                if not p._noclipConns then p._noclipConns = {} end
                p._noclipConns[char] = conn
            end
        else
            local char = getChar()
            if char then
            ...
            if p._noclipConns then
                for char, conn in pairs(p._noclipConns) do
                    if conn then conn:Disconnect() end
                end
                p._noclipConns = {}
            end
    ...
    createToggle(p, y, "无限跳跃", function() return FuncState.InfiniteJump end, function(v)
        FuncState.InfiniteJump = v
        if v then
            local char = getChar()
            if char and char:FindFirstChildOfClass("Humanoid") then
                local hum = char.Humanoid
                hum.JumpPower = FuncState.JumpPower
                hum:SetStateEnabled(Enum.HumanoidStateType.Jumping, true)
                local conn
                conn = RunService.Heartbeat:Connect(function()
                    if not FuncState.InfiniteJump then
                        if conn then conn:Disconnect() end
                        return
                    end
                ...
                if not p._infJumpConns then p._infJumpConns = {} end
                p._infJumpConns[char] = conn
            end
        else
            if p._infJumpConns then
                for char, conn in pairs(p._infJumpConns) do
                    if conn then conn:Disconnect() end
                end
                p._infJumpConns = {}
            end
    ...
    y = y + 46
    createSlider(p, y, "跳跃高度 (50-200)", 50, 200, 100, function(v)
        FuncState.JumpPower = v
        if FuncState.InfiniteJump then
            local char = getChar()
            if char and char:FindFirstChildOfClass("Humanoid") then
    ...
    y = y + 46
    createToggle(p, y, "墙体检测", function() return FuncState.WallCheck end, function(v)
        FuncState.WallCheck = v
    end)
...
task.spawn(function()
    ...
    local function mutateParams()
        params.jumpProbBase = math.random(30, 70) / 100
        params.jumpMultMin = math.random(12, 18) / 10
        params.jumpMultMax = math.random(20, 30) / 10
        params.angleDev = math.random(8, 25)
        params.standStillProb = math.random(5, 18) / 100
        params.backStepProb = math.random(5, 15) / 100
        params.backStepMult = math.random(30, 55) / 100
    end

    mutateParams()

    RS.RenderStepped:Connect(function(dt)
        if not FuncState.AntiDetect then
            return
        end
        ...
        pcall(function()
            ...
            if (shakeTimer >= math.random(15, 30)) then
                shakeTimer = 0
                mutateParams()
            end

            local currentSpeed = HRP.Velocity.Magnitude
            local targetSpeed = (FuncState.SpeedEnabled and FuncState.SpeedValue) or currentSpeed

            if ((currentSpeed > 180) or (Hum.WalkSpeed > 180)) then
                local dir = getMoveDirection()

                if (not dir or (dir.Magnitude < 0.01)) then
                    dir = Vector3.new((math.random() - 0.5) * 2, 0, (math.random() - 0.5) * 2)

                    if (dir.Magnitude < 0.01) then
                        dir = HRP.CFrame.LookVector * 0.1
                    end
                ...
                local mode = FuncState.Mode or 1

                if (mode == 1) then
                    local idealStep = math.min(targetSpeed * dt, 8)
                    local jumpProb = params.jumpProbBase + ((math.random() - 0.5) * 0.2)
                    jumpProb = math.clamp(jumpProb, 0.1, 0.9)

                    if (math.random() < jumpProb) then
                        local mult = params.jumpMultMin + (math.random() * (params.jumpMultMax - params.jumpMultMin))
                        local step = idealStep * mult
                        local angle = math.rad((math.random() - 0.5) * 2 * params.angleDev)
                        local rot = CFrame.Angles(0, angle, 0)
                        local jumpDir = (rot * Vector3.new(dir.X, 0, dir.Z)).Unit

                        if (jumpDir.Magnitude < 0.01) then
                            jumpDir = dir
                        end

                        displacement = jumpDir * step

                        if (math.random() < params.backStepProb) then
                            displacement = -jumpDir * idealStep * params.backStepMult
                        end
                    elseif (math.random() < params.standStillProb) then
                        displacement = Vector3.zero
                    else
                        displacement = Vector3.new((math.random() - 0.5) * 0.2, (math.random() - 0.5) * 0.1, (math.random() - 0.5) * 0.2)
                    end
                elseif (mode == 2) then
                    local step = math.min(targetSpeed * dt, 2.5)
                    local jitter = Vector3.new((math.random() - 0.5) * 0.15, (math.random() - 0.5) * 0.05, (math.random() - 0.5) * 0.15)
                    displacement = (dir * step) + jitter
                elseif (mode == 3) then
                    modeCounter = modeCounter + 1

                    if ((modeCounter % 2) == 0) then
                        local step = math.min(targetSpeed * dt * 2, 6)
                        local jitter = Vector3.new((math.random() - 0.5) * 0.3, (math.random() - 0.5) * 0.1, (math.random() - 0.5) * 0.3)
                        displacement = (dir * step) + jitter
                    else
                        displacement = Vector3.zero
                    end
                ...
                if (displacement.Magnitude > 10) then
                    displacement = displacement.Unit * 10
                end
                ...
                if (#positionHistory > HISTORY_MAX) then
                    table.remove(positionHistory, 1)
                end

                frameCounter = frameCounter + 1

                if (frameCounter >= math.random(8, 18)) then
                    frameCounter = 0
                    local offset = math.random(1, 4)

                    if (#positionHistory > offset) then
                        local oldPos = positionHistory[#positionHistory - offset]
                        HRP.Position = oldPos
                        HRP.Velocity = Vector3.zero
                        HRP.AssemblyLinearVelocity = Vector3.zero
                        positionHistory = {}
            ...
            if _G.BeingWatched then
                _G.BeingWatched = nil
            end

            if shared.BeingWatched then
                shared.BeingWatched = nil
            end

            if getgenv().BeingWatched then
                getgenv().BeingWatched = nil
            end
        end)
    end)
...
task.spawn(function()
    while true do
        ...
        if FuncState.AntiDetect then
            local newMode = math.random(1, 3)

            while newMode == FuncState.Mode do
                newMode = math.random(1, 3)
            end

            FuncState.Mode = newMode
            print("[防检测] 切换至模式 " .. newMode)
        end
    end
end)
...
local function updateHitboxVisuals()
    safeCall(function()
        local enabled = FuncState.HitboxEnabled
        local scale = FuncState.HitboxSize / 10
        ...
        for _, plr in ipairs(Players:GetPlayers()) do
            if (plr ~= LocalPlayer) then
                ...
                if (enabled and root and char) then
                    local extents = char:GetExtentsSize()
                    local boxSize = extents * scale
                    local visual = hitboxVisuals[plr]

                    if (not visual or (visual.Parent ~= root)) then
                        if visual then
                            visual:Destroy()
                        end
        ...
        for plr, visual in pairs(hitboxVisuals) do
    ...
    while task.wait(0.016) do
        if FuncState.BypassAC then
            pcall(function()
                ...
                if getgenv then
                    getgenv().BeingWatched = nil
                    getgenv().AntiCheat = nil
                    getgenv().Detected = nil
                    getgenv().AntiCheatLevel = 0
                    getgenv().DetectScore = 0

                    for k, v in pairs(getgenv()) do
                        if ((type(k) == "string") and (string.find(string.lower(k), "detect") or string.find(string.lower(k), "watch") or string.find(string.lower(k), "check") or string.find(string.lower(k), "anticheat") or string.find(string.lower(k), "admin"))) then
                            getgenv()[k] = nil
                        end
                    end
                end
                ...
                if shared then
                    shared.BeingWatched = nil
                    shared.AntiCheat = nil
                    shared.Detected = nil
                    shared.AntiCheatLevel = 0
                    shared.DetectScore = 0
...
task.spawn(function()
    ...
    while task.wait(0.1) do
        if FuncState.FlyCarEnabled then
                        ...
                        if model then
                            ...
                            if part then
                                ...
                                if (moveDir.Magnitude > 0) then
                                    local vel = moveDir.Unit * flySpeed
                                    vehiclePart.CFrame = vehiclePart.CFrame + vel
                                end

                                local currentY = vehiclePart.Position.Y
                                vehiclePart.CFrame = CFrame.new(vehiclePart.Position.X, currentY, vehiclePart.Position.Z) * vehiclePart.CFrame.Rotation
                            end
                        end
                    elseif flyCarActive then
                        if bodyGyro then
                            bodyGyro:Destroy()
                        end
